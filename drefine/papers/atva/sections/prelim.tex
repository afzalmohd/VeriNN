...
\item If $Type_i= \relu$ and $y=\relu(x)$, where $x$ is a neuron in $l_{i-1}$ and $y$ is a neuron in $l_i$, we consider the following three cases: %\todo{I tried to simplify the notation, please have a look}:
  \begin{enumerate}
