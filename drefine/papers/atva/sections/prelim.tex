...
\item If $Type_i= \affine$, we set 
  ...
  {\color{red} \todo{Added explaination by running example} Consider the neuron $x_3$ in Figure~\ref{fig:motivating}. 
...
\item If $Type_i= \relu$ and $y=\relu(x)$,{\color{red}\todo{As per comment 1.5.5 of reviewer 2} where $x$ is a neuron in $l_{i-1}$ and $y$ is a neuron in $l_i$}, we consider the following three cases: %\todo{I tried to simplify the notation, please have a look}:
  \begin{enumerate}
  ...
  \item  If $A(x).lb < 0$ and $A(x).ub > 0$,
    ...
    {\color{red} Consider the neuron $x_5$ in Figure~\ref{fig:motivating}, whose input is $x_3$. 
...
{\color{red}\todo{With respect to comment 3 of reviewer 2} Furthermore, Algorithm~\ref{algo:verif1} includes a function called \getmodel{}, 
