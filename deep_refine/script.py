...
def print_cmnds_bounds(num_cpu, log_dir):
    ...
    for idx, load in enumerate(load_per_cpu):
        ...
        for l in ld:
            ...
            command = f"taskset --cpu-list {idx}-{idx} timeout -k 2s {timeout} {TOOL} --network {net_path} --vnnlib-prp-file {prp_path} --bounds-path {bounds_path} --dataset {DATASET} --result-file {result_file} >> {log_file}"
            cmds.append(command)
    ...
    for image_index in range(NUM_IMAGES):
        for ep in epsilons:
            for nt in NETWORK_FILE:
                tasks.append([os.path.join(net_dir, nt), ep, image_index])
...
if __name__ == '__main__':
    ...
    print_cmnds_bounds(num_cpu, log_dir)  
