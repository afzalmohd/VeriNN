...
\begin{itemize}
  \item $n_{ij}$ represents the $j^{th}$ neuron of $i^{th}$ layer. 
  ...
  \item $val_{ij}$ represents the $j^{th}$ value of value vector $\boldsymbol{val_{i}}$
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    \State $\boldsymbol{y'} = N(\boldsymbol{x})$
    \State \textbf{return} $\boldsymbol{x}$ if $\boldsymbol{y} \nvDash Q$. 
   \For{$j=0$ to $l_k.size$}
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    \State $\boldsymbol{y'} = N(\boldsymbol{x})$
    \State \textbf{return} $\boldsymbol{x}$ if $\boldsymbol{y} \nvDash Q$. 
    \State Let us say $\boldsymbol{val_{i}}$ is the value vector of $l_i$, when $\boldsymbol{x}$ executed on network $N$. 
