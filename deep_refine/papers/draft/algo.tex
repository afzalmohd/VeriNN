  ...
  \begin{algorithmic}[1]
   ...
   \For{$i=k$ to $1$}
      \If{$l_i$ is affine layer}
        ...
        \For{$j=0$ to $l_i.size$}
          \State $constr_{i,j}$ = $(A(x_{i,j},lexpr)$ == $satval_{i,j}$) \Comment{lexpr=rexpr for affine layer}
          \State layerConstrains.append($constr_{i,j}$)
        \EndFor
        % \For{$j=0$ to $l_{i-1}.size$}\Comment{m is size of layer $l_{i-1}$}
        %   \State layerConstrains.append$(A(x_{i,j}, lb))$
        %   \State layerConstrains.append$(A(x_{i,j}, ub))$
        % \EndFor
        \State isSat = checkSat(And(layerConstrains)) \Comment{And of all constraints}
        \If{isSat}
      ...
      \Else \Comment{Relu layer}
        \For{$j=0$ to $l_i.size$}
          ...
          \Else
            \State $A(x_{i-1,j},lb) \leq satval_{i-1,j} \leq$ 0 \Comment{lb,ub are bounds}
          \EndIf
        \EndFor
      \EndIf
   \EndFor
    \State ce = \{$satval_{0,i}$ | $i=0$ to $l_{0}.size$ \} \Comment{If pullbacked upto input layer}
