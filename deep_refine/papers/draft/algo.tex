...
A input vector $\boldsymbol{v} \in \reals^{|l_0|}$ is a counter example(cex) if $N(\boldsymbol{v}) \models \lnot Q$.  
...
\begin{df}
  ...
  $uexpr \in LinExpr$ is the expression for the upper bound.\todo{Define abstraction and execution under abstraction}
...
\begin{df}
  ...
  spurious counter example(spuriousCEX) if $\boldsymbol{v_0}$  and is not a counterexample.
