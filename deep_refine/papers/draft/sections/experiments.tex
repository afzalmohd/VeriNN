...
% Table~\ref{tb:matrix1} shows the pairwise comparison of the verifiers on adversarially trained networks, the total number of benchmarks for 
% defended networks are $2223$. We are performing far better than \deeppoly{}, \kpoly{}, and \deepsrgr{} on these benchmarks. 
% In comparison to \alphabeta{} and \ovaltool{}, although they are solving around 135 more benchmarks in comparison to our approach. 
% But we are solving around $75$ unique benchmarks which are not solved by both the tools. Out of $2223$ the number 
% $75$ is a reasonable number of benchmarks. 
...
\paragraph{Comparison with preprocessing in algorithm~\ref{algo:main}: } 
...
Our experiments further emphasize that the DREFINE technique we use is orthogonal to most other techniques.\todo{I tried to summarize the reviews related to experiments. } 
