We have implemented our approach in a prototype and compared it to three types of related approaches 
...
respectively\footnote{We could not compare with 2nd rank tool \textsc{VeriNet}, since we had difficulties installing its external solver's (\textsc{Xpress}) license. Also, since we are comparing with \deeppoly{} and \kpoly{}, and \textsc{ERAN} uses these techniques internally, so we skip to compare with \textsc{ERAN}, which achieved 4th rank in VNN-COMP'21}, 
...
we do\footnote{\alphabeta{} outperforms compared to \marabou{} in VNN-COMP'21, while in our experiment \marabou{} is doing better than \alphabeta{}, a potential reason could be \alphabeta{} uses \textsc{GPU}, while \marabou{} uses \textsc{CPU} in VNN-COMP'21.}. 
...
\begin{table}[t]
  \scriptsize
    ...
    \begin{tabular}{|c|c|c|c|c|c|c|c|g||c|}
        ...
        \hline
    \end{tabular}
    \caption{Pairwise comparison of tools, e.g. the entry on row \kpoly{} and column \deeppoly{} represents 156 benchmark instances on which \kpoly{} verified but \deeppoly{} fails. The green row represents the number of solved benchmark instances by \drefine{} and not by other, the red column represents green's opposite.}
    \label{tb:matrix}
\end{table}
