...
They keep the back substitute  up to the input layer. The process of back substitution is shown in equation (1). %~\ref{eq:deeppoly}.
...
There are two main reasons for the failure of \deeppoly{}. One is it cannot maintain the complete correlation 
...
affine node (as done in~\cite{?})\todo{add citation?} or by using the exact encoding (eq~\ref{eq:reluexact}) 
...
so, the input $x_1=1, x_2=1$ is a spurious counter-example. 
...
Here, the black line represents the spurious counter-example denoted by equation~\ref{eq:sat1}. 
The green line represents the exact execution of the input point of spurious counter-example 
...
The divergence we find by the \texttt{maxsat} query. If \texttt{maxsat} returns all the soft constraints as satisfied, it means
the blue point becomes equal to the green point. If \texttt{maxsat} returns partial soft constraints as satisfied, 
we mark the neurons whose soft constraints are not satisfied. In our example, \texttt{maxsat} returns 
